    ...
    <div>
      <div>
        <div class="queryCondition-top">
          <div class="queryCondition-title queryTitle">新闻收藏夹(新)</div>
          <div class="middle clearFloat">
            ...
            <!-- 查询按钮 -->
            <div class="queryBtndropdel">
              <span id="del" @click="del">删除</span>
              <span @click="drop">清空</span>
            </div>
              ...
              <tbody>
                ...
                <tr v-for="(item, index) of dataList" :key="index">
                 ...
                 <td class="starBox">
                  ...
                  <td class="tableTd">{{item.publishtime}}</td>
                  <td class="data-content">{{item.compnames[0]}}</td>
                  <td>{{item.secnames[0]}}</td>
...
export default {
  data() {
    ...
    //   },
      ...
      titleData: ['标题', '重要性', '发布时间', '公司名称', '证券', '详情'],
      dataList: [],
      firstTypeList: [
        { type: '全部', check: false },
  ...
  methods: {
    ...
    paginationSelect(pageNumber) {
      const stock_url = "http://10.25.26.194:10201/api/risk/finchina/news_favorite_query"
      const stockSendData = {
          userid : 'risk',
          sectype : 'B',
          page :pageNumber-1,
      ...
      let formData = new FormData()
      for(let key in stockSendData){
          if(stockSendData.hasOwnProperty(key)){
              formData.append(key,stockSendData[key])
          }
      }
      this.$_axios.post(stock_url,formData).then(res=>{
          console.log(res)
          this.isShowQueryResult = true
          this.hasResultData = true
          if(res.data.code == 0){
              const resultDatas = res.data.result
              resultDatas.forEach(item=>{
                  item.check = false
              })
              this.dataList = JSON.parse(JSON.stringify(resultDatas))
                this.paginationData.page_Count = Math.ceil(res.data.totalcount/10);
    ...
    // }
    del(){
        const delurl = 'http://10.25.24.51:10201/api/risk/finchina/news_favorite_del'
            let delID = []
            this.dataList.forEach(item=>{
                if(item.check == true){
                    delID.push(item.ID)
        ...
        let formData = new FormData()
        for(let key in delsendData){
            if(delsendData.hasOwnProperty(key)){
                formData.append(key,delsendData[key])
            }
        }
        this.$_axios.post(delurl,formData).then(res=>{
            console.log(res)
            if(res.data.code == 0){
                alert('删除成功')
                this.stockCollec()
    ...
    drop(){
        const dropurl = 'http://10.25.24.51:10201/api/risk/finchina/news_favorite_drop'
        ...
        let formData = new FormData()
        for(let key in dropsendData){
            if(dropsendData.hasOwnProperty(key)){
                formData.append(key,dropsendData[key])
            }
        }
        this.$_axios.post(dropurl,formData).then(res=>{
            console.log(res)
            if(res.data.code == 0){
                alert('清空成功')
                this.stockCollec()
