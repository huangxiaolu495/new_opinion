    ...
    <div>
      <div @click='inputNone' >
        <div class="queryCondition-top">
          <div class="queryCondition-title">债券发行人负面新闻(新)</div>
...
export default {
  data() {
    ...
    return {
      url: 'http://10.25.24.51:10201/api/risk/finchina/primary_news_bond?',
  ...
  methods: {
    ...
    inputmethod(){
      ...
      if(nowNumber > 99999){
        ...
        console.log(this.mathCompanyb)
        console.log(this.nowQuite.toString())
        this.mathCompanyb.forEach(item => {
          if(item.compname){
    ...
    companyMatch(){
      ...
      this.$_axios.get(matchUrl,{
        ...
        console.log(res)
        this.mathCompanyb = res.data.result
      })
    },
    mask_close(){
    ...
    export_mask(){
      ...
      let keyWords = this.exportData.keyword
      ...
      window.location.href = 'http://10.25.24.51:10201/api/risk/finchina/primary_news_export?sectype='+sectype+'&start_date='+timestart+'&end_date='+timeend+'&keytype='+keyType+'&keyword='+keyWords+'&scope='+scope+'&userid='+userId+'&sector='+sector+'&imp='+imp+'&riskcode='+riskcode+'&newscodes='+newsCodes+'&export_type='+exportType
    ...
    moduleCheckAlldown(item,index){
      ...
      let allxuan = this.dataList.every(item=>{
        return item.check
      })
      if(allxuan){
    ...
    checkRiskType() {
      ...
      if (this.isShowTypeList) {
        ...
        console.log(this.secondTypeList)
        console.log(this.firstTypeList)
...
.pullDownListContent .pullDownListBox ul li:nth-child(3){
   margin-left: 10px;
}
</style>
<style>
